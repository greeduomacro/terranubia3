	...
	public class TownHouseSign : Item
	{
		...
		public TimeSpan RentByTime
		{
			...
			set
			{
				...
				{
					Name = "This building has been marked for sale.";
					ClearRentTimer();
				}
				else
				{
					Name = "This building has been marked for rent.";
					ClearRentTimer();
		...
		public void ShowAreaPreview()
		{
			ShowAreaPreview( -1000 );
		}

		public void ShowAreaPreview( int z )
		{
			ClearPreview();
			...
			foreach( Point2D p in blocks )
			{
				...
				item.Movable = false;
				item.Location = new Point3D( p.X, p.Y, (z == -1000 ? Map.GetAverageZ( p.X, p.Y )+2 : z ) );
				item.Map = Map;
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( c_Free )
				list.Add( 1060658, "Price\tFree" ); // ~1_val~: ~2_val~
			else if ( c_RentByTime == TimeSpan.Zero )
				list.Add( 1060658, "Price\t{0}{1}", c_Price, c_KeepItems ? " (+" + c_ItemsPrice + " for the items)" : "" ); // ~1_val~: ~2_val~
			else if ( c_RecurRent )
				list.Add( 1060658, "{0}\t{1}\r{2}", PriceType + (c_RentToOwn ? " Rent-to-Own" : " Recurring"), c_Price, c_KeepItems ? " (+" + c_ItemsPrice + " for the items)" : "" ); // ~1_val~: ~2_val~
			else
				list.Add( 1060658, "One {0}\t{1}{2}", PriceTypeShort, c_Price, c_KeepItems ? " (+" + c_ItemsPrice + " for the items)" : "" ); // ~1_val~: ~2_val~

			list.Add( 1060659, "Lockdowns\t{0}", c_Locks ); // ~1_val~: ~2_val~
			list.Add( 1060660, "Secures\t{0}", c_Secures ); // ~1_val~: ~2_val~

			if ( c_SkillReq != 0.0 )
				list.Add( 1060661, "Requires\t{0}", c_SkillReq + " in " + c_Skill ); // ~1_val~: ~2_val~
			if ( c_MinTotalSkill != 0 )
				list.Add( 1060662, "Requires more than\t{0} total skills", c_MinTotalSkill ); // ~1_val~: ~2_val~
			if ( c_MaxTotalSkill != 0 )
				list.Add( 1060663, "Requires less than\t{0} total skills", c_MaxTotalSkill ); // ~1_val~: ~2_val~

			if ( c_YoungOnly )
				list.Add( 1063483, "Must be\tYoung" ); // ~1_val~: ~2_val~
			else if ( c_Murderers == Intu.Yes )
				list.Add( 1063483, "Must be\ta murderer" ); // ~1_val~: ~2_val~
			else if ( c_Murderers == Intu.No )
				list.Add( 1063483, "Must be\tinnocent" ); // ~1_val~: ~2_val~
		}
